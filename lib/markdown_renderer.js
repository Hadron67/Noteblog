    ...
    compileMarkdown(file, data, resolve, reject){
        ...
        if (frontMatter.layout){
            if (this.ctx.layouts.hasOwnProperty(frontMatter.layout)){
                ...
                content = cp.render(this.ctx.layouts[frontMatter.layout], param);
            }
            else {
...
class MarkdownHandler {
    constructor(ctx, renderer, inFile, path){
        ...
        ctx.watch(inFile, (event) => event === 'change' && this.refreshMarkdown());
    }
    refreshMarkdown(){
        this.ctx.readFile(this.inFile, (err, data) => {
            if (err){
                ...
                this.renderer.compileMarkdown(this.inFile, data.toString('utf-8'), a => {
                    this.article = a;
                    this.ctx.emit('Markdown', a);
                    this.ctx.logger.info(`Markdown file ${this.inFile} compiled`);
                    if (this.getPath){
