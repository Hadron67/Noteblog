...
class Node {
    ...
    static makeLinkNode(name, href, attrs){
    ...
    static makeImageNode(alt, src, attrs){
...
class VisitorBase {
    ...
    enterArticle(node)    { return this.defaultEnter(node); }
    leaveArticle(node)    { return this.defaultLeave(node); }
    enterHeading(node)    { return this.defaultEnter(node); }
    leaveHeading(node)    { return this.defaultLeave(node); }
    enterParagraph(node)  { return this.defaultEnter(node); }
    leaveParagraph(node)  { return this.defaultLeave(node); }
    enterQuoteBlock(node) { return this.defaultEnter(node); }
    leaveQuoteBlock(node) { return this.defaultLeave(node); }
    enterList(node)       { return this.defaultEnter(node); }
    leaveList(node)       { return this.defaultLeave(node); }
    enterListItem(node)   { return this.defaultEnter(node); }
    leaveListItem(node)   { return this.defaultLeave(node); }
    enterTable(node)      { return this.defaultEnter(node); }
    leaveTable(node)      { return this.defaultLeave(node); }
    enterTableHead(node)  { return this.defaultEnter(node); }
    leaveTableHead(node)  { return this.defaultLeave(node); }
    enterTableBody(node)  { return this.defaultEnter(node); }
    leaveTableBody(node)  { return this.defaultLeave(node); }
    enterTableRow(node)   { return this.defaultEnter(node); }
    leaveTableRow(node)   { return this.defaultLeave(node); }
    enterTableCol(node)   { return this.defaultEnter(node); }
    leaveTableCol(node)   { return this.defaultLeave(node); }
    enterHtmlTag(node)    { return this.defaultEnter(node); }
    leaveHtmlTag(node)    { return this.defaultLeave(node); }
}
class HTMLConverter {
    constructor(classes) {
        this.text = '';
    ...
    visitImage(node) {
        this.text += `<img src="${node.src}"${node.attrs.length > 0 ? ` attrs="${node.attrs.join(' ')}"` : ''} alt="${node.alt}" />`; 
    }
        ...
        return tryParse(() => {
            if (token === '['){
                ...
                if (token === '('){
                    ...
                    return image ? new ImageNode(desc, src, attrs) : Node.makeLinkNode(desc, src, attrs);
                }
            }
            return false;
        });
