...
class SassHandler {
    ...
    refresh(cb){
        this.ctx.readFile(this.files[0], (err, data) => {
            if (err){
                this.logger.err(`Failed to read sass file ${this.files[0]}: ${err.toString()}`);
                cb && cb(err, this);
            }
            else {
                this.renderer.compileFile(this.files[0], css => { 
                    this.result = css;
                    this.logger.info(`Sass file ${this.files[0]} compiled`);
                    cb && cb(false, this);
                }, err => {
                    this.logger.err(`Failed to parse sass file ${this.files[0]}: ${err.toString()}`);
                    cb && cb(err, this);
                });
            }
        });
