...
class Handler {
    ...
    handle(os, cb){
        ...
        for (let page of app.extend.blogManager.getPosts()){
            ...
            page.getTags().forEach(t => d.leaf('category', {scheme: app.ext.tags + app.helper.regulateName(t) + '/', term: t}));
            let categoryPath = page.getCategory().join('/');
            categoryPath && d.leaf('category', {scheme: app.ext.category + app.helper.regulateName(categoryPath) + '/', term: categoryPath});
            d.pop();
            count++;
...
module.exports = params => app => {
    ...
    app.register(params.path, new Handler(app, params.path, params.limit));
