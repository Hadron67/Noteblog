...
class Handler {
    ...
    handle(os, cb){
        ...
        for (let page of app.extend.blogManager.getPosts()){
            ...
            page.getTags().forEach(t => d.leaf('category', {scheme: pathd.join(app.ext.tags, app.helper.regulateName(t), '/'), term: t}));
            let categoryPath = page.getCategory().join('/');
            categoryPath && d.leaf('category', {scheme: pathd.join(app.ext.category, app.helper.regulateName(categoryPath), '/'), term: categoryPath});
            d.pop();
            count++;
...
module.exports = params => app => {
    ...
    app.on('init-pages', () => {
        app.pageRegistry.register(params.path, new Handler(app, params.path, params.limit));
    });
}