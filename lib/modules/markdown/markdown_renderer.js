...
class MarkdownContext extends event.EventEmitter {
    ...
    compileMarkdown(file, data, renderer, resolve, reject){
    ...
    register(path, inFile, arg = {}, renderer = null){
        return new MarkdownHandler(this.ctx, this, inFile, path, arg, renderer || this.renderer);
    }
    dateToPath(pathBase){
        return article => {
            let d = article.date;
            ...
            let dataPath = `${d.getFullYear()}/${mouth}/${date}/${changeSuffix(article.fileName, '')}/index.html`;
...
class MarkdownHandler extends event.EventEmitter {
    ...
    refresh(cb){
        this.ctx.readFile(this.inFile, (err, data) => {
            ...
            if (err){
                this.logger.err(`Failed to read markdown file ${this.inFile}: ${err.toString()}`);
                cb && cb(err, this);
            }
            else if (data === ''){
                this.logger.warn(`Skipping empty markdown file ${this.inFile}`);
            }
            else {
                this.rendererContext.compileMarkdown(this.inFile, data, this.renderer, a => {
                    this.article = a;
                    this.emit('compiled', this);
                    if (this.rendererContext.applyFilter(a)){
                        this.filtered = true;
                        if (this.path){
                            this.rendererContext.logger.info(`Unregistering filtered post ${this.inFile}`);
                            this.ctx.unregister(this.path);
                            this.path = null;
                            this.emit('remove', this);
                        }
                        else {
                            this.rendererContext.logger.info(`Skipping filtered post ${this.inFile}`);
                        }
