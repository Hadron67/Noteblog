...
module.exports = params => async (app) => {
    ...
    let ext = app.ext;
    ext.blog = app.path(params.mainPage.path);
    ext.tags = postArg.tags;
    ext.archive = postArg.archive;
    ext.category = postArg.category;

    let mainPage = app.helper.createPaginator(compareDate, arg => app.layouts.page(arg), indexToPageName, params.mainPage.pagesPerPage, ext.blog, {arg: postArg});
