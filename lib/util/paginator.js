...
class Paginator {
    ...
    register(path, pages){
        this.ctx.pageRegistry.registerTemplate(path, this.template, merge({pages, path}, this.arg));
        return this;
    }
    add(page){
    ...
    update(){
        ...
        while (this._lastPaths.length < pageCount){
            ...
            let path0 = path.join(this.pathBase, this.pathTemplate(i + 1));
            this._lastPaths.push(path0);
            this.register(path0, new PagesFragment(this._lastPaths, i, this.pages, i * this.pagesPerPage, (i + 1) * this.pagesPerPage));
        }
        while (this._lastPaths.length > pageCount){
            this.ctx.pageRegistry.unregister(this._lastPaths.pop());
        }
