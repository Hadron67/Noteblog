...
function createMainContext(){
    ...
    function registerModule(src, cb){
        function refresh(){
            fs.readFile(src, (err, data) => {
                if (err){
                    logger.err(`Cannot read module ${src}: ${err.toString()}`);
                }
                else {
                    try {
                        let func = new Function('main', data.toString('utf-8'));
                        func(main);
                        logger.info(`Module ${src} loaded`);
                        if (cb){
                            cb(main);
                            cb = null;
                        }
                        ...
                        logger.err(`Error while loading module ${src}: ${e.toString()}`);
                    }
                }
            });
        }
        refresh();
        watch(src, (event, fn) => event === 'change' && refresh());
    }

    // Internal methods
    main.registerRaw = registerRaw;
    main.registerTemplate = registerTemplate;
    main.registerModule = registerModule;
