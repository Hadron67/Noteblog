...
![笑脸与哭脸对消](/asset/2016/img/turning-nombre.png)
...
![三维等高线以及高度的变化趋势](/asset/2016/img/3d-mountains.png)
其中蓝色线代表高度减小的方向，红色代表增加。
为了把三维情形类比到四维空间中，我们画出曲胞的“等高面”。这样类比出来的等高面有两种：一种是一簇同心球，另一种是一簇圆锥面和双曲面。
![等高线的两种四维类比](/asset/2016/img/hyperbolics-et-spheres.png)
...
![四维等高面](/asset/2016/img/4d-mountains.png)
...
![峰与峰和峰与鞍点靠近时的等高线](/asset/2016/img/interactions.png)
...
$$
TurningNumber={1\over S_{n}}\int_{M}K dA
$$
其中$dA$就是超曲面上的超面积元，$S\_{n}$是n维单位超球的面积，它可以用一种巧妙的方法可以得到，<a class="show-and-hide" href="javascript:void(0);" target="_self" data-target-id="hide1">点这里</a>显示这种方法。
...
$$
\begin{align}
(\sqrt{\pi})^n&=(\int_{-\infty}^{+\infty}e^{-x^2}dx)^n=\int d^n x\exp(-\sum_{i=0}^n x_{n}^2)\\
&=\int d\Omega_{n}\int_{0}^{+\infty}dx x^{n-1}e^{-x^2}\\
&=(\int d\Omega_{n})\int_{0}^{+\infty}{1\over 2}d(x^2)(x^2)^{n/2-1}e^{-x^2}\\
&=(\int d\Omega_{n}){1\over 2}\Gamma({n\over 2})
\end{align}
$$
上面的$\int d\Omega_{n}$就是我们想要的$S_n$，所以
$$
